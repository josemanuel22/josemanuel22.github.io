...
Memory is managed by a stop-the-world, non-moving, markand-sweep garbage collector [3,4]. The mark phase can be executed in parallel. The collector has a single old generation for objects that survive a number of cycles. It uses a shadow stack [5] to record pointers in order to be precise.
...
This is a summary of the ideas developed in the famous work [Bezanson, Jeff, et al. "Julia: Dynamism and performance reconciled by design." Proceedings of the ACM on Programming Languages 2.OOPSLA (2018): 1-23.](https://dl.acm.org/doi/pdf/10.1145/3276490)
