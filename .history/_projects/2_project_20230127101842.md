...
- *Bernoulli Trial*. A event can either be a birth or a death. So we need to determine which event actually occured. Choose a random number between $$(0,1)$$ and see if it is less than $$\frac{d\cdot N}{\beta + d\cdot N}$$ or bigger. In each case we add or subtract an individual to the population (Monte-Carlo simulation).
