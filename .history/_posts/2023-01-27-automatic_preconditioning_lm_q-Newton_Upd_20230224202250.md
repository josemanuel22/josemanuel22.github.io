...
\begin{algorithm}[H]
\caption{Automatic Preconditioning limited Memory Quasi-Newton Updating}
  \begin{algorithmic}[1]
    \State Solve $\{x_1\}$, $\{r_1\}$ $\leftarrow A_{1}x = b_{1}$ with unpreconditioned CG-method;
   % \State Get  generated by the CG-method;
    \For{$i=2, ..., k$}
