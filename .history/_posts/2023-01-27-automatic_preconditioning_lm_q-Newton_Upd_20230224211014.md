...
{% highlight c++ linenos %}
  \begin{algorithmic}[1]
  \State Let $i=0$ and $x_i=x_0$ be our initial guess;
  \While{Stoping test not satisfied}
   \State At $x_{n}$ compute $\nabla f(x_{n})$ and $H(f)(x_{n})$ and consider taylor expansion of $f$:
   \begin{align*}
   f(x+\Delta x ) \approx f(x) + \nabla f(x)^{T} \Delta x + \Delta x^{T} H(f) \Delta x.
\end{align*}    
\State Compute $x_{n+1}$  with C.G. for quadratic functions on the Taylor expansion.
      \EndWhile
{% endhighlight %}
