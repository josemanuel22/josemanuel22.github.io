...
- Two possible events: births (or synthesis) and deaths (or decays).$$\beta_{birth} = (constant), \quad \beta_{death}  = (k_0)*n.$$
...
- *pure birth process*, the governing equation is: 
$$n(t)=b\cdot t + n(0)$$. 

So any numerical solution that we obtain has to follow a linear pattern of $\beta$ slope, for this case.

- *pure death process* the  governing equation is: 
$$n(t)=e^{-d\cdot t}+n(0)-1$$. 

In this case the decreasing exponential pattern has to be observed.
\end{itemize}
