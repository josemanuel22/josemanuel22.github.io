...
for(i=2, ..., k) {
    Compute/store: $s_{i}=x_{i+1}-x_{i}, \quad y_{i} =r_{i+1}-r_{i}, \quad i = l_1, ..., l_{m}$;
    Define BFGS matrix $H_{k}$ using $\{s_{i}\}, \{y_{i}\}$; (quasi-Newton preconditioner)
    $\{x_i\}$, $\{r_i\}$ $\leftarrow$ Use $H_{k}$ preconditioned CG-method to solve $A_{i}x = b_{i}$;
