...
Solve $\{x_1\}$, $\{r_1\}$ $\leftarrow A_{1}x = b_{1}$ with unpreconditioned CG-method;
    \For{$i=2, ..., k$}
        \State Compute/store: $s_{i}=x_{i+1}-x_{i}, \quad y_{i} =r_{i+1}-r_{i}, \quad i = l_1, ..., l_{m}$;
      \State Define BFGS matrix $H_{k}$ using $\{s_{i}\}, \{y_{i}\}$; (quasi-Newton preconditioner)
      \State $\{x_i\}$, $\{r_i\}$ $\leftarrow$ Use $H_{k}$ preconditioned CG-method to solve $A_{i}x = b_{i}$;
    \EndFor
{% endhighlight %}
